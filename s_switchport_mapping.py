...
def script_main(script):
    ...
    int_table = get_int_status(script)

    mac_table = get_mac_table(script)

    desc_table = get_desc_table(script)
    ...
    output_filename = script.create_output_filename("PortMap", ext=".csv")
...
def get_int_status(script):
    ...
    if script.os == "IOS":
        template_file = script.get_template("cisco_ios_show_interfaces_status.template")
    else:
        template_file = script.get_template("cisco_nxos_show_interface_status.template")

    raw_int_status = script.get_command_output(send_cmd)
...
def get_mac_table(script):
    ...
    if script.os == "IOS" and len(mac_table) == 1:
    ...
    # Check for vPCs on NXOS to account for "vPC Peer-Link" entries in MAC table of N9Ks
        ...
        send_cmd = "show vpc"
        vpc_template = script.get_template("cisco_nxos_show_vpc.template")

        raw_show_vpc = script.get_command_output(send_cmd)
